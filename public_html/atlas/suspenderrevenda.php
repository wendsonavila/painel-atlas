...
//verifica se esta com ssl ou nao
$loop = Factory::create();
$servidores_com_erro = [];
$sucess = false;         
...
if ($sucess == true) {
   echo "1";
    foreach ($contas as $conta) {
        //print_r($conta);
        //update para suspenco
        $sql5 = "UPDATE ssh_accounts SET mainid = 'Suspenso' WHERE byid = '$conta[id]'";
        $result5 = $conn->query($sql5);
        $sql6 = "UPDATE atribuidos SET suspenso = '1' WHERE userid = '$conta[id]'";
        $result6 = $conn->query($sql6);
    }
    foreach ($deletes as $delete) {
        //print_r($delete);
       $sql5 = "UPDATE ssh_accounts SET mainid = 'Suspenso' WHERE byid = '$delete[id]'";
       $result5 = $conn->query($sql5);
       $sql6 = "UPDATE atribuidos SET suspenso = '1' WHERE userid = '$delete[id]'";
         $result6 = $conn->query($sql6);
   }  
    foreach ($dells as $dell) {
        //print_r($dell);
       $sql5 = "UPDATE ssh_accounts SET mainid = 'Suspenso' WHERE byid = '$dell[id]'";
       $result5 = $conn->query($sql5);
       $sql6 = "UPDATE atribuidos SET suspenso = '1' WHERE userid = '$dell[id]'";
         $result6 = $conn->query($sql6);
    } 
    echo "<script>sweetAlert('Sucesso!', 'Revendedor Suspenso com sucesso!', 'success').then(function() { window.location.href = 'listarrevendedores.php'; });</script>";
}else{
    echo "<script>swal('Erro!', 'Erro ao Suspender revendedor!', 'error').then(function() { window.location.href = 'listarrevendedores.php'; });</script>";
} 
$loop->run();
unlink($nome);
